...
class DctElaTransform:
    """
    Apply DCT transformation (high-frequency components) and ELA analysis on the input image,
    and add DCT and ELA results as additional channels to the image.
    
    Attributes:
        quality (int): Quality for JPEG compression used in ELA.
        
    Methods:
        __call__: Apply DCT and ELA on the input image and store results in additional channels.
    """
    
    def __init__(self, quality=90):
        """
        Initialize DctElaTransform object for DCT and ELA.
        
        Args:
            quality (int): Quality level for JPEG compression in ELA.
        """
        self.quality = quality

    def __call__(self, labels):
        """
        Apply DCT and ELA transformations to the input image and add the results as extra channels.

        Args:
            labels (Dict): A dictionary containing the image and any other associated labels.

        Returns:
            Dict: The updated labels dictionary with the DCT and ELA results as additional channels.
        """
        img = labels["img"]  # Retrieve the image from the dictionary
        
        # Apply DCT and ELA transformations
        dct_high = self.apply_dct(img)
        ela_img = self.apply_ela(img)
        hsv_img = self.apply_hsv(img)

        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        
        # Convert DCT and ELA results to match image shape (H, W, C)
        if img.ndim == 3:
            h, w, _ = img.shape
            dct_high = dct_high[:,:,np.newaxis]  
            ela_img = ela_img[:,:,np.newaxis]
        
        # Ensure the channels are aligned properly for appending
        img = cv2.normalize(img, None, 0, 255, cv2.NORM_MINMAX)
        dct_high = cv2.normalize(dct_high, None, 0, 255, cv2.NORM_MINMAX)
        ela_img = cv2.normalize(ela_img, None, 0, 255, cv2.NORM_MINMAX)
        
        # Stack DCT and ELA channels along the channel axis (C)
        img_with_extra = np.concatenate([img, dct_high[:,:,np.newaxis], ela_img[:,:,np.newaxis]], axis=-1)

        # Update the image with the new channels
        labels["img"] = img_with_extra

        return labels

    def apply_dct(self, image):
        """
        Apply DCT transformation and extract high-frequency components from the image.
        
        Args:
            image (np.ndarray): Input image as a numpy array.
        
        Returns:
            np.ndarray: The high-frequency components from the DCT of the image.
        """
        ...
        if (H % block_size != 0) or (W % block_size != 0):
            gray = cv2.copyMakeBorder(gray, 0, block_size - H % block_size, 0, block_size - W % block_size, cv2.BORDER_CONSTANT, value=0)
            H_, W_ = gray.shape[:2]
        else:
            H_, W_ = H, W

        gray = gray.reshape(H_ // block_size, block_size, W_ // block_size, block_size).transpose(0, 2, 1, 3)
        gray = gray.reshape(-1, block_size, block_size).astype(np.float32)

        dct_blocks = [cv2.dct(block) for block in gray]
        idct_blocks= []
        for block in dct_blocks:
            block[:block.shape[0]//2, :block.shape[1]//2] = 0
            idct_blocks.append(cv2.idct(block))
        res = np.array(idct_blocks).reshape(H_ // block_size, W_ // block_size, block_size, block_size).transpose(0, 2, 1, 3)
        res = res.reshape(H_, W_)
        res = cv2.resize(res, (W, H), interpolation=cv2.INTER_LINEAR)
    
        return res

    def apply_ela(self, image, quality=90):
        """
        Apply ELA (Error Level Analysis) to an image.
        ...
        Returns:
            np.ndarray: The ELA image showing differences between original and compressed image.
        """
        encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), quality]
        _, img_encoded = cv2.imencode('.jpg', image, encode_param)
        img_compressed = cv2.imdecode(img_encoded, 1)
        
        # Calculate the absolute difference between original and compressed image
        ela_image = cv2.absdiff(image, img_compressed)
        ela_image = cv2.cvtColor(ela_image, cv2.COLOR_BGR2GRAY)
        
        return ela_image
    
    def apply_hsv(self, image):
        """
        Apply HSV transformation to an image.
        
        Args:
            image (np.ndarray): Input image as a numpy array.
        
        Returns:
            np.ndarray: The HSV-transformed image.
        """
