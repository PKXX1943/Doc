...
class BaseModel(nn.Module):
    ...
    def load(self, weights, verbose=True, ckpt_idx=None):
        """
        ...
        if ckpt_idx is not None:
            ...
            for key, value in csd.items():
                layer_idx = int(key.split('.')[1])  # 'model.0.conv' -> 0
                if layer_idx < len(ckpt_idx):
                    new_layer_idx = ckpt_idx[layer_idx]
                else:
                    raise ValueError(f"Original layer index {layer_idx} exceeds the new sequence length.")
                
                new_key = key.replace(f'model.{layer_idx}', f'model.{new_layer_idx}')
                renamed_csd[new_key] = value
            csd = renamed_csd
